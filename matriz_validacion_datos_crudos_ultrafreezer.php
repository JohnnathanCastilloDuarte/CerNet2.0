<?php 
	...
	$query_2 = mysqli_prepare($connect,"SELECT b.nombre, b.id_sensor, a.id_ultrafreezer_sensor, a.posicion FROM ultrafreezer_sensor as a, sensores as b WHERE a.id_mapeo = ? AND a.id_asignado = ? AND a.id_sensor = b.id_sensor ORDER BY LENGTH(a.posicion) ASC, a.posicion ASC");
	mysqli_stmt_bind_param($query_2, 'ii', $id_mapeo, $id_asignado);
	mysqli_stmt_execute($query_2);
	mysqli_stmt_store_result($query_2);
	mysqli_stmt_bind_result($query_2,  $nombre_sensor, $id_sensor, $id_ultrafreezer_sensor, $posicion);
	...
	while($row = mysqli_stmt_fetch($query_2)){
		...
		echo "<td style='color:red' > Pos-$posicion <br>$nombre_sensor</td>";
	}
echo "</tr>";
	...
	mysqli_stmt_fetch($query_21);
      ...
      $query_31 = mysqli_prepare($connect,"SELECT  a.temperatura, b.posicion FROM ultrafreezer_datos_crudos  as a, ultrafreezer_sensor as b, sensores as c WHERE  a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? AND b.id_sensor = c.id_sensor ORDER BY a.time ASC,  LENGTH(b.posicion) ASC, b.posicion ASC");
      mysqli_stmt_bind_param($query_31, 'i', $id_mapeo);
      mysqli_stmt_execute($query_31);
      mysqli_stmt_store_result($query_31);
      mysqli_stmt_bind_result($query_31, $datos, $posicion_dc);
		...
		for($j=1;$j<=$filas;$j++){
			...
			for($g=1;$g<=$cuantos;$g++){
				mysqli_stmt_fetch($query_31);
					echo "<td><strong>$posicion_dc</strong><br>$datos</td>";
			}
