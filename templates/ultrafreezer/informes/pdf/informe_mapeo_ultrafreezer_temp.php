<?php
    ...
    /////////////////////////////////////////////////////////PASOS DE CREACIÓN DE PDF///////////////////////////////////////////////////////////
		...
		$query_1 = mysqli_prepare($connect,"SELECT nombre, id_asignado, id_mapeo, concepto, observacion, comentarios,  fecha_registro, n_increment FROM informe_ultrafreezer WHERE id_informe_ultrafreezer = ?");
		mysqli_stmt_bind_param($query_1, 'i', $id_informe);
		mysqli_stmt_execute($query_1);
		mysqli_stmt_store_result($query_1);
		mysqli_stmt_bind_result($query_1, $dato_1, $id_asignado, $id_mapeo, $concepto, $observacion, $comentarios, $fecha_registro, $n_increment);
		mysqli_stmt_fetch($query_1);
		$nombre_informe = $dato_1.''.$n_increment;
  ...
  $query_participante = mysqli_prepare($connect, "SELECT nombres, apellido, cargo FROM informes_participante WHERE id_numot = ?");
  mysqli_stmt_bind_param($query_participante, 'i', $id_numot);
  mysqli_stmt_execute($query_participante);
  mysqli_stmt_store_result($query_participante);
  mysqli_stmt_bind_result($query_participante, $nombres_p, $apellidos_p, $cargo_p);
  mysqli_stmt_fetch($query_participante);
		...
		$a = "PRUEBA DE DISTRIBUCIÓN TERMICA DE ULTRAFREEZER";
		...
		$query_6 = mysqli_prepare($connect,"SELECT a.nombre, a.descripcion, b.fabricante, b.modelo, b.n_serie, b.c_interno, b.ubicacion 
																			  FROM item as a, item_ultrafreezer as b WHERE a.id_item = b.id_item AND a.id_item = ?");
		...
		$query_7 = mysqli_prepare($connect,"SELECT valor_seteado_temperatura, temperatura_minima, temperatura_maxima, fecha_inicio, hora_inicio,
																				fecha_final, hora_final,intervalo	FROM ultrafreezer_mapeo WHERE id_asignado = ? AND id_mapeo = ?");
		...
		$query_8 = mysqli_prepare($connect,"SELECT COUNT(a.temperatura) / COUNT(DISTINCT b.id_sensor) as x  FROM ultrafreezer_datos_crudos as a , ultrafreezer_sensor as b 
																				WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_asignado = ? AND b.id_mapeo = ?");
		...
		$query_9 = mysqli_prepare($connect,"SELECT DISTINCT MAX(CAST(a.temperatura AS DECIMAL(6,2))) as maximo, a.time FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b 
																				WHERE CAST(a.temperatura AS DECIMAL(6,2)) >= ? AND b.id_asignado = ? AND a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor  AND b.id_mapeo = ?
																				GROUP BY a.time");
		mysqli_stmt_bind_param($query_9, 'iii', $temperatura_max, $id_asignado, $id_mapeo);
      ...
      $query_10 = mysqli_prepare($connect,"SELECT DISTINCT MAX(CAST(a.temperatura AS DECIMAL(6,2))) as maximo, a.time FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b 
                                          WHERE b.id_asignado = ? AND a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor 
                                          GROUP BY a.time");
		...
		$query_11 = mysqli_prepare($connect,"SELECT DISTINCT MIN(CAST(a.temperatura AS DECIMAL(6,2))) as minimo, a.time FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b 
																				WHERE CAST(a.temperatura AS DECIMAL(6,2)) <= ? AND b.id_asignado = ? AND a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor  AND b.id_mapeo = ?
																				GROUP BY a.time");
		mysqli_stmt_bind_param($query_11, 'sii', $temperatura_min, $id_asignado, $id_mapeo);
    ...
    if(mysqli_stmt_num_rows($query_11) == 0){
      $min_percent = 0;
    }else{
    ...
    if(mysqli_stmt_num_rows($query_9) == 0 or $c_hora == 0){
      ...
      $max_percent = 0;
		...
		$query_12 = mysqli_prepare($connect,"SELECT nombre FROM ultrafreezer_mapeo WHERE id_mapeo = ?");
		...
		if( $concepto == "concepto"){
			$resultado_corresponde = $nombre_mapeo." para ".$nombre_item." por un periodo de ".$c_hora ."hora(s).";	
		}else{
		...
		$query_13 = mysqli_prepare($connect,"SELECT AVG(CAST(a.temperatura AS DECIMAL(6,2))) as promedio FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b 
																					WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? ");
		...
		$query_14 = mysqli_prepare($connect,"SELECT MAX(CAST(a.temperatura AS DECIMAL(6,2))) as maximo, a.time, c.nombre, d.nombre, b.posicion FROM ultrafreezer_datos_crudos as a,
																					ultrafreezer_sensor as b, bandeja as c, sensores as d WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? 
																					AND b.id_asignado = ?
		...
		$query_15 = mysqli_prepare($connect,"SELECT MIN(CAST(a.temperatura AS DECIMAL(6,2))) as minimo, a.time, c.nombre, d.nombre, b.posicion FROM ultrafreezer_datos_crudos as a,
																					ultrafreezer_sensor as b, bandeja as c, sensores as d WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? 
																					AND b.id_asignado = ?
		...
		$query_16 = mysqli_prepare($connect,"SELECT STD(CAST(a.temperatura AS DECIMAL(6,2))) as desviacion FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b, 
																					bandeja as c, sensores as d WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ?
	...
	$query_18 = mysqli_prepare($connect,"SELECT MAX(CAST(temperatura AS DECIMAL(6,2))) AS maximo, MIN(CAST(temperatura AS DECIMAL(6,2))) as minimo, MAX(CAST(temperatura AS DECIMAL(6,2))) - MIN(CAST(temperatura AS DECIMAL(6,2))) AS resta,
																				a.time FROM ultrafreezer_datos_crudos as a,
																					ultrafreezer_sensor as b, bandeja as c, sensores as d WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ?
																					AND b.id_asignado = ?
  ...
  $convert_dife_max = number_format($dif_maxi,2);
  $convert_dife_min = number_format($dif_min,2);


 
	$query_19 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, ultrafreezer_sensor as b, ultrafreezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_ultrafreezer_sensor = c.id_ultrafreezer_sensor 
																			 AND  CAST(c.temperatura AS DECIMAL (6,2)) =  ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
	mysqli_stmt_bind_param($query_19, 'ssii', $convert_dife_max, $dif_max_time, $id_asignado, $id_mapeo);
    ...
    $query_23 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, ultrafreezer_sensor as b, ultrafreezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_ultrafreezer_sensor = c.id_ultrafreezer_sensor 
                                         AND  CAST(c.temperatura AS DECIMAL (6,2)) = ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
    mysqli_stmt_bind_param($query_23, 'ssii', $convert_dife_min, $dif_max_time, $id_asignado, $id_mapeo);
	...
	$query_24 = mysqli_prepare($connect,"SELECT MAX(CAST(temperatura AS DECIMAL(6,2))) AS maximo, MIN(CAST(temperatura AS DECIMAL(6,2))) as minimo, MAX(CAST(temperatura AS DECIMAL(6,2))) - MIN(CAST(temperatura AS DECIMAL(6,2))) AS resta,
																				a.time FROM ultrafreezer_datos_crudos as a,
																					ultrafreezer_sensor as b, bandeja as c, sensores as d WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ?
																					AND b.id_asignado = ?
  ...
  $convert_dif_maxi_2 = number_format($dif_maxi_2,2);
  $convert_dif_min_2 = number_format($dif_min_2, 2);


    $query_25 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, ultrafreezer_sensor as b, ultrafreezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_ultrafreezer_sensor = c.id_ultrafreezer_sensor 
                                         AND  CAST(c.temperatura AS DECIMAL (6,2)) = ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
    mysqli_stmt_bind_param($query_25, 'ssii', $convert_dif_maxi_2, $dif_max_time_2, $id_asignado, $id_mapeo);
    ...
    mysqli_stmt_fetch($query_25);
     ...
     $query_26 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, ultrafreezer_sensor as b, ultrafreezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_ultrafreezer_sensor = c.id_ultrafreezer_sensor 
                                         AND  CAST(c.temperatura AS DECIMAL (6,2)) = ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
    mysqli_stmt_bind_param($query_26, 'ssii', $convert_dif_min_2, $dif_max_time_2, $id_asignado, $id_mapeo);
	...
	$query_27 = mysqli_prepare($connect,"SELECT AVG(CAST(a.temperatura AS DECIMAL(6,2))) as promedio, c.nombre, d.nombre, b.posicion FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b, bandeja as c, sensores as d 
																				WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? AND c.id_bandeja = b.id_bandeja AND d.id_sensor = b.id_sensor
																				GROUP BY c.nombre ,d.nombre, b.posicion order by promedio desc limit 1 ");
	mysqli_stmt_bind_param($query_27, 'ii', $id_mapeo ,$id_asignado);
	mysqli_stmt_execute($query_27);
	mysqli_stmt_store_result($query_27);
	mysqli_stmt_bind_result($query_27, $d_4, $max_avg_posicion, $max_avg_sensor, $posicion_max_avg);
	mysqli_stmt_fetch($query_27);

	$max_avg = number_format($d_4,2);

	//SENSORES CON PROMEDIO MAS BAJO 
	$query_28 = mysqli_prepare($connect,"SELECT AVG(CAST(a.temperatura AS DECIMAL(6,2))) as promedio, c.nombre, d.nombre, b.posicion FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b, bandeja as c, sensores as d 
																				WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? AND c.id_bandeja = b.id_bandeja AND d.id_sensor = b.id_sensor
																				GROUP BY c.nombre ,d.nombre, b.posicion order by promedio ASC limit 1 ");
	mysqli_stmt_bind_param($query_28, 'ii', $id_mapeo ,$id_asignado);
	mysqli_stmt_execute($query_28);
	mysqli_stmt_store_result($query_28);
	mysqli_stmt_bind_result($query_28, $d_5, $min_avg_posicion, $min_avg_sensor, $posicion_min_avg);
	mysqli_stmt_fetch($query_28);
	
	$min_avg = number_format($d_5,2);

	//SENSOR CON MAYOR DESVIACION
	$query_29 = mysqli_prepare($connect,"SELECT std(CAST(a.temperatura AS DECIMAL(6,2))) as promedio, c.nombre, d.nombre, b.posicion FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b, bandeja as c, sensores as d 
																				WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? AND c.id_bandeja = b.id_bandeja AND d.id_sensor = b.id_sensor
																				GROUP BY c.nombre ,d.nombre, b.posicion order by promedio DESC limit 1 ");
	mysqli_stmt_bind_param($query_29, 'ii', $id_mapeo ,$id_asignado);
	mysqli_stmt_execute($query_29);
	mysqli_stmt_store_result($query_29);
	mysqli_stmt_bind_result($query_29, $d_6, $max_desv_posicion, $max_desv_sensor, $posicion_desv_max);
	mysqli_stmt_fetch($query_29);
  
	$max_desv = number_format($d_6,2);
	
	//SENSOR CON MENOR DESVIACION
	$query_30 = mysqli_prepare($connect,"SELECT std(CAST(a.temperatura AS DECIMAL(6,2))) as promedio, c.nombre, d.nombre, b.posicion FROM ultrafreezer_datos_crudos as a, ultrafreezer_sensor as b, bandeja as c, sensores as d 
																				WHERE a.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? AND c.id_bandeja = b.id_bandeja AND d.id_sensor = b.id_sensor
																				GROUP BY c.nombre ,d.nombre, b.posicion order by promedio ASC limit 1 ");
	mysqli_stmt_bind_param($query_30, 'ii', $id_mapeo ,$id_asignado);
	mysqli_stmt_execute($query_30);
	mysqli_stmt_store_result($query_30);
	mysqli_stmt_bind_result($query_30, $d_7, $min_desv_posicion, $min_desv_sensor, $posicion_desv_min);
	mysqli_stmt_fetch($query_30);

	$min_desv = number_format($d_7,2);


  $imagen1 = "no-image.png";
  $imagen2 = "no-image.png";
  $imagen3 = "no-image.png";

  //OBTENER UBICACIÓN DE SENSORES Y SUS IMAGENES 
   
  $query_31 = mysqli_prepare($connect,"SELECT ubicacion FROM images_informe_ultrafreezer WHERE id_informe = ? AND tipo_imagen = ?");
  ...
  if($ubicacion_posicion_sensores != ""){   
    $imagen1 = $ubicacion_posicion_sensores;
  }


  $query_34 = mysqli_prepare($connect,"SELECT ubicacion FROM images_informe_ultrafreezer WHERE id_informe = ? AND tipo_imagen = 2");
  ...
  mysqli_stmt_fetch($query_34);
   ...
   if($img_sensores_2 != ""){
    $imagen2  = $img_sensores_2;
  }


  $query_35 = mysqli_prepare($connect,"SELECT ubicacion FROM images_informe_ultrafreezer WHERE id_informe = ? AND tipo_imagen = 3");
...
<table><tr><td bgcolor="#DDDDDD"><H3><strong>INSPECCIÓN DE DISTRIBUCIÓN TERMICA</strong></H3></td></tr></table><br><br>
<table>
...
<tr><td width="15%"><strong>Solicitante:</strong></td><td>$nombre_empresa</td>
		...
		<tr><td width="15%"><strong>Atención:</strong></td><td>$nombres_p $apellidos_p - $cargo_p</td>
		...
		<tr><td width="30%" class="enunciado">N° de serie / Código interno</td><td width="70%">$n_serie_item / $c_interno_item </td></tr>
		<tr><td width="30%" class="enunciado">Ubicación</td><td width="70%">$ubicacion_item</td></tr>
		<tr><td width="30%" class="enunciado">Valor seteado (°C)</td><td width="70%">$valor_seteado </td></tr>
		...
		<td width="10%">a las:</td><td width="20%">$max_time_general</td><td width="10%">En:</td><td width="20%">$sensor_max_general, Ubicado en posición: $posicion_max_general</td></tr>

		<tr><td width="30%" class="enunciado">Mínimo General (°C)</td><td width="10%">$min_general</td>
		<td width="10%">a las:</td><td width="20%">$min_time_general</td><td width="10%">En:</td><td width="20%">$sensor_min_general, Ubicado en posición: $posicion_min_general</td></tr>
	  ...
	  <br><br>
		...
		<tr><td width="30%" class="enunciado">Sensor con promedio más alto (°C)</td><td width="10%">$max_avg</td><td width="10%">en:</td><td width="50%">$max_avg_sensor, ubicado en: $posicion_max_avg</td></tr>

		<tr><td width="30%" class="enunciado">Sensor con promedio más bajo (°C)</td><td width="10%">$min_avg</td><td width="10%">en:</td><td width="50%">$min_avg_sensor, ubicado en: $posicion_min_avg</td></tr>

		<tr><td width="30%" class="enunciado">Sensor con mayor Desv. Est. (°C)</td><td width="10%">$max_desv</td><td width="10%">en:</td><td width="50%">$max_desv_sensor, ubicado en: $posicion_desv_max</td></tr>

		<tr><td width="30%" class="enunciado">Sensor con menor Desv. Est. (°C)</td><td width="10%">$min_desv</td><td width="10%">en:</td><td width="50%">$min_desv_sensor, ubicado en: $posicion_desv_min</td></tr>
</table>
...
<tr><td><br><br> <img src="../../$imagen1"> </td></tr></table><br><br><br>
EOD;
...
$pdf->writeHTMLCell(30, 5, 120, '', 'N° de serie', 1, 0, 0, true, 'C', true);

$pdf->writeHTMLCell(45, 5, 150, '', 'N° Certificado de Calibración', 1, 1, 0, true, 'C', true);

$contador_t = 0;
//CONSULTA
$query_32 = mysqli_prepare($connect,"SELECT  c.posicion, a.nombre, b.nombre, a.serie, a.id_sensor  FROM sensores as a, bandeja as b, ultrafreezer_sensor as c WHERE c.id_sensor = a.id_sensor AND c.id_mapeo = ? AND b.id_bandeja = c.id_bandeja ORDER BY b.nombre ASC, LENGTH(c.posicion) ASC, c.posicion ASC");
mysqli_stmt_bind_param($query_32, 'i', $id_mapeo);
mysqli_stmt_execute($query_32);
mysqli_stmt_store_result($query_32);
mysqli_stmt_bind_result($query_32, $posicion, $nombre_sensor_t, $ubicacion_sensor_t, $serie_sensor_t,  $id_sensor_t);

while($row = mysqli_stmt_fetch($query_32)){
      $contador_t ++;
 $query_34 = mysqli_prepare($connect,'SELECT certificado FROM sensores_certificados WHERE id_sensor = ? ORDER BY fecha_vencimiento DESC');
  mysqli_stmt_bind_param($query_34, 'i', $id_sensor_t);
  mysqli_stmt_execute($query_34);
  mysqli_stmt_store_result($query_34);
  mysqli_stmt_bind_result($query_34, $certificado_sensor_t);
  while($row = mysqli_stmt_fetch($query_34)){
    if($contador_t == 30 OR $contador_t == 60 ){
      ...
      $pdf->writeHTMLCell(30, 5, 120, '', $serie_sensor_t, 1, 0, 0, true, 'C', true);

      $pdf->writeHTMLCell(45, 5, 150, '', $certificado_sensor_t, 1, 1, 0, true, 'C', true);
...
$pdf->writeHTMLCell(35, 10, 90, '', 'Desv. Estándar', 1, 0, 0, true, 'C', true);
...
$query_33 = mysqli_prepare($connect,"SELECT DISTINCT a.nombre, d.nombre, c.posicion, MIN(CAST(b.temperatura AS DECIMAL(6,2))) as Minimo, 
                                    ...
                                    SUM(CASE WHEN CAST(b.temperatura AS DECIMAL(6,2))> ? THEN 1 ELSE 0 END) as tiempo_over, 
                                    SUM(CASE WHEN CAST(b.temperatura AS DECIMAL(6,2))< ? THEN 1 ELSE 0 END) as tiempo_low,
                                    AVG(EXP(-83.144/(0.0083144*(CAST(b.temperatura AS DECIMAL(6,2))+273.15)))) as valor FROM sensores as a,
                                    ultrafreezer_datos_crudos as b, ultrafreezer_sensor as c, bandeja as d WHERE a.id_sensor = c.id_sensor AND c.id_ultrafreezer_sensor = b.id_ultrafreezer_sensor AND c.id_mapeo = ?  AND d.id_bandeja = c.id_bandeja GROUP BY a.nombre, d.nombre, c.posicion ORDER BY d.nombre ASC, c.posicion ASC");
mysqli_stmt_bind_param($query_33, 'ssi', $temperatura_max, $temperatura_min, $id_mapeo);
mysqli_stmt_execute($query_33);
mysqli_stmt_store_result($query_33);
mysqli_stmt_bind_result($query_33, $nombre_sensor_t_2, $bandeja, $posicion, $minimo_t, $maximo_t, $promedio_t, $desviacion_t, $tiempo_over_t, $tiempo_low_t, $valor);
...
if($info_over>$c_hora)
{
$info_over=number_format(($tiempo_over_t*$intervalo)/3600,0).".00";
$info_percent_over="100.00";
}

if($info_low>$c_hora)
{
$info_low=number_format(($tiempo_low_t*$intervalo)/3600,0).".00";
$info_percent_low="100.00";
}	
$mkt=number_format(-1*(83.144/0.0083144)/(log($valor_mkt))-273.15,2);	
  
$pdf->writeHTMLCell(25, 10, 15, '', $posicion.'-'.$nombre_sensor_t_2, 1, 0, 0, true, 'C', true);
...
$pdf->writeHTMLCell(35, 10, 90, '', $info_desv, 1, 0, 0, true, 'C', true);
...
<tr><td><br><br><img src="../../$imagen2"></td></tr></table><br><br><br>
<table>
...
<tr><td><br><br><img src="../../$imagen3"></td></tr></table><br><br><br>
<table>
...
<tr><td height="90"><br><br><br>Ing. Raúl Quevedo Silva <br>Auditor Regulatorio- Cercal Group Spa.</td><td height="50"></td></tr>
</table>
