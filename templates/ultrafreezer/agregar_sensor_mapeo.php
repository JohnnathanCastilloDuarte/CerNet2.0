<?php 
		...
		$consultar = mysqli_prepare($connect,"SELECT id_ultrafreezer_sensor FROM ultrafreezer_sensor WHERE id_asignado = ? AND id_mapeo =  ? AND id_sensor = ?");
		mysqli_stmt_bind_param($consultar, 'iii', $id_asignado, $id_mapeo,  $id_sensor);
		mysqli_stmt_execute($consultar);
		mysqli_stmt_store_result($consultar);
		mysqli_stmt_bind_result($consultar, $id_ultrafreezer_sensor);
		mysqli_stmt_fetch($consultar);



		if($id_ultrafreezer_sensor){
			echo "Existe";
...
*/			
			...
			$ingresar = mysqli_prepare($connect,"INSERT INTO ultrafreezer_sensor (id_asignado, id_bandeja, id_sensor, id_mapeo, id_usuario)
																						VALUES (?,?,?,?,?) ");
