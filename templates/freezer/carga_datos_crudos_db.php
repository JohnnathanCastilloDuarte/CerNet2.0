<?php 
	include("../../config.ini.php");
		...
		//VALIDAR SI EXISTE EL CAMPO PARA INGRESAR LA INFORMACIÒN
			...
			$validar = mysqli_prepare($connect,"SELECT id_freezer_sensor FROM freezer_datos_crudos WHERE id_freezer_sensor = $id_freezer_sensor");
		...
		//BUSCAR FECHAS DE RANGO DE MAPEO
			//Recuperar fechas 
		$fechas = mysqli_prepare($connect,"SELECT fecha_inicio, hora_inicio, fecha_final, hora_final, intervalo FROM freezer_mapeo WHERE id_mapeo = ?");
			...
			if(file_exists($datos_crudos))
						...
						$abre_archivo=fopen($datos_crudos,'r');
							...
							if($fecha_hora_sql>=$start && $fecha_hora_sql<=$end && $fecha_hora_sql <> ""){
								...
								$insertar = mysqli_prepare($connect,"INSERT INTO freezer_datos_crudos (id_freezer_sensor, time, temperatura,humedad, id_usuario) VALUES (?,?,?,?,?)");
								mysqli_stmt_bind_param($insertar, 'isssi', $id_freezer_sensor, $fecha_suma, $temperatura, $humedad, $id_valida);
...
/*
	...
	$buscar_datos = mysqli_prepare($connect,"SELECT id_sensor, id_bandeja FROM freezer_sensor WHERE id_freezer_sensor = $id_freezer_sensor");
	...
	$insertando_historial = mysqli_prepare($connect,"INSERT INTO historial_freezeres (id_usuario, mensaje_historial, tipo_historial) VALUES ( ?, ?, ?)");
