<?php
    ...
    $posicion_sensores_indicativo = 1;
		...
		$query_1 = mysqli_prepare($connect,"SELECT nombre, id_asignado, id_mapeo, concepto, observacion, comentarios FROM informe_freezer WHERE id_informe_freezer = ?");
		...
		$a = "DETERMINACIÓN TERMICA DE HUMDEDAD";
		...
		$query_7 = mysqli_prepare($connect,"SELECT valor_seteado_humedad, humedad_minima, humedad_maxima, fecha_inicio, hora_inicio,
																				fecha_final, hora_final,intervalo	FROM freezer_mapeo WHERE id_asignado = ? AND id_mapeo = ?");
		...
		$query_8 = mysqli_prepare($connect,"SELECT COUNT(a.humedad) / COUNT(DISTINCT b.id_sensor) as x  FROM freezer_datos_crudos as a , freezer_sensor as b 
																				WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_asignado = ? AND b.id_mapeo = ?");
		...
		$query_9 = mysqli_prepare($connect,"SELECT DISTINCT MAX(CAST(a.humedad AS DECIMAL(6,2))) as maximo, a.time FROM freezer_datos_crudos as a, freezer_sensor as b 
																				WHERE CAST(a.humedad AS DECIMAL(6,2)) >= ? AND b.id_asignado = ? AND a.id_freezer_sensor = b.id_freezer_sensor 
																				GROUP BY a.time");
		...
		$query_10 = mysqli_prepare($connect,"SELECT DISTINCT MAX(CAST(a.humedad AS DECIMAL(6,2))) as maximo, a.time FROM freezer_datos_crudos as a, freezer_sensor as b 
																				WHERE b.id_asignado = ? AND a.id_freezer_sensor = b.id_freezer_sensor 
																				GROUP BY a.time");
		...
		$query_11 = mysqli_prepare($connect,"SELECT DISTINCT MIN(CAST(a.humedad AS DECIMAL(6,2))) as minimo, a.time FROM freezer_datos_crudos as a, freezer_sensor as b 
																				WHERE CAST(a.humedad AS DECIMAL(6,2)) <= ? AND b.id_asignado = ? AND a.id_freezer_sensor = b.id_freezer_sensor 
																				GROUP BY a.time");
		...
		$query_12 = mysqli_prepare($connect,"SELECT nombre FROM freezer_mapeo WHERE id_mapeo = ?");
		...
		$query_13 = mysqli_prepare($connect,"SELECT AVG(CAST(a.humedad AS DECIMAL(6,2))) as promedio FROM freezer_datos_crudos as a, freezer_sensor as b 
		...
		$query_14 = mysqli_prepare($connect,"SELECT MAX(CAST(a.humedad AS DECIMAL(6,2))) as maximo, a.time, c.nombre, d.nombre FROM freezer_datos_crudos as a,
																					freezer_sensor as b, bandeja as c, sensores as d WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_mapeo = ? 
																					AND b.id_asignado = ?
		...
		$query_15 = mysqli_prepare($connect,"SELECT MIN(CAST(a.humedad AS DECIMAL(6,2))) as minimo, a.time, c.nombre, d.nombre FROM freezer_datos_crudos as a,
																					freezer_sensor as b, bandeja as c, sensores as d WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_mapeo = ? 
																					AND b.id_asignado = ?
		...
		$query_16 = mysqli_prepare($connect,"SELECT STD(CAST(a.humedad AS DECIMAL(6,2))) as desviacion FROM freezer_datos_crudos as a, freezer_sensor as b, 
	...
	$query_17 = mysqli_prepare($connect,"SELECT AVG(EXP(-83.144/(0.0083144*(CAST(humedad AS DECIMAL(6,2))+273.15)))) as valor FROM freezer_datos_crudos as a, freezer_sensor as b, 
																					bandeja as c, sensores as d WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ?
																					AND b.id_bandeja = c.id_bandeja AND b.id_sensor = d.id_sensor  ");
	...
	$query_18 = mysqli_prepare($connect,"SELECT MAX(CAST(humedad AS DECIMAL(6,2))) AS maximo, MIN(CAST(humedad AS DECIMAL(6,2))) as minimo, MAX(CAST(humedad AS DECIMAL(6,2))) - MIN(CAST(humedad AS DECIMAL(6,2))) AS resta,
																				a.time FROM freezer_datos_crudos as a,
																					freezer_sensor as b, bandeja as c, sensores as d WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_mapeo = ?
																					AND b.id_asignado = ?
	...
	$query_19 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, freezer_sensor as b, freezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_freezer_sensor = c.id_freezer_sensor 
	...
	$query_20 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, freezer_sensor as b, freezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_freezer_sensor = c.id_freezer_sensor 
																			 AND c.humedad = ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
	...
	$query_22 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, freezer_sensor as b, freezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_freezer_sensor = c.id_freezer_sensor 
																			 AND c.humedad = ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
	...
	$query_23 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, freezer_sensor as b, freezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_freezer_sensor = c.id_freezer_sensor 
																			 AND c.humedad = ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
	...
	$query_25 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, freezer_sensor as b, freezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_freezer_sensor = c.id_freezer_sensor 
																			 AND c.humedad = ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
	...
	$query_26 = mysqli_prepare($connect,"SELECT a.nombre FROM sensores as a, freezer_sensor as b, freezer_datos_crudos as c WHERE a.id_sensor = b.id_sensor AND b.id_freezer_sensor = c.id_freezer_sensor 
																			 AND c.humedad = ? AND c.time = ? AND b.id_asignado = ? AND b.id_mapeo = ? ");
	...
	$query_27 = mysqli_prepare($connect,"SELECT AVG(CAST(a.humedad AS DECIMAL(6,2))) as promedio, c.nombre, d.nombre FROM freezer_datos_crudos as a, freezer_sensor as b, bandeja as c, sensores as d 
																				WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? AND c.id_bandeja = b.id_bandeja AND d.id_sensor = b.id_sensor
	...
	$query_28 = mysqli_prepare($connect,"SELECT AVG(CAST(a.humedad AS DECIMAL(6,2))) as promedio, c.nombre, d.nombre FROM freezer_datos_crudos as a, freezer_sensor as b, bandeja as c, sensores as d 
																				WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? AND c.id_bandeja = b.id_bandeja AND d.id_sensor = b.id_sensor
	...
	$query_29 = mysqli_prepare($connect,"SELECT std(CAST(a.humedad AS DECIMAL(6,2))) as promedio, c.nombre, d.nombre FROM freezer_datos_crudos as a, freezer_sensor as b, bandeja as c, sensores as d 
																				WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? AND c.id_bandeja = b.id_bandeja AND d.id_sensor = b.id_sensor
																				GROUP BY c.nombre ,d.nombre order by promedio DESC limit 1 ");
	...
	$query_30 = mysqli_prepare($connect,"SELECT std(CAST(a.humedad AS DECIMAL(6,2))) as promedio, c.nombre, d.nombre FROM freezer_datos_crudos as a, freezer_sensor as b, bandeja as c, sensores as d 
																				WHERE a.id_freezer_sensor = b.id_freezer_sensor AND b.id_mapeo = ? AND b.id_asignado = ? AND c.id_bandeja = b.id_bandeja AND d.id_sensor = b.id_sensor
																				GROUP BY c.nombre ,d.nombre order by promedio ASC limit 1 ");
  ...
  $query_31 = mysqli_prepare($connect,"SELECT ubicacion FROM images_informe_freezeres WHERE id_informe = ? AND tipo_imagen = ?");
  ...
  $query_34 = mysqli_prepare($connect,"SELECT ubicacion FROM images_informe_freezeres WHERE id_informe = ? AND tipo_imagen = 2");
  ...
  $query_35 = mysqli_prepare($connect,"SELECT ubicacion FROM images_informe_freezeres WHERE id_informe = ? AND tipo_imagen = 3");
...
$contador_t = 0;
//CONSULTA
$query_32 = mysqli_prepare($connect,"SELECT DISTINCT a.nombre, b.nombre, a.serie, a.id_sensor  FROM sensores as a, bandeja as b, freezer_sensor as c WHERE c.id_sensor = a.id_sensor AND c.id_mapeo = ? AND b.id_bandeja = c.id_bandeja");
...
$query_33 = mysqli_prepare($connect,"SELECT DISTINCT a.nombre, MIN(CAST(b.humedad AS DECIMAL(6,2))) as Minimo, 
                                    ...
                                    freezer_datos_crudos as b, freezer_sensor as c WHERE a.id_sensor = c.id_sensor AND c.id_freezer_sensor = b.id_freezer_sensor AND c.id_mapeo = ? GROUP BY a.nombre");
