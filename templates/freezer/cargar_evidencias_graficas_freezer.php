<?php 
	...
	$primera = mysqli_prepare($connect,"SELECT id_mapeo, id_asignado FROM informe_freezer WHERE id_informe_freezer = $id_informe");
...
$target_dir = "Evidencia_Grafica/Ot_".$id_numot."/Id_servicio_".$id_asignado."/Id_mapeo_".$id_mapeo."/Posicion_sensores/";
	...
	$target_file = $target_dir.basename($_FILES["imagen_1_freezer"]["name"]);
	$uploadOk = 1;
	$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
	// Check if image file is a actual image or fake image

	$check = getimagesize($_FILES["imagen_1_freezer"]["tmp_name"]);
		if($check !== false) {
		...
		// Check file size
		if ($_FILES["imagen_1_freezer"]["size"] > 5000000) {
		...
		// if everything is ok, try to upload file
			...
			if (move_uploaded_file($_FILES["imagen_1_freezer"]["tmp_name"], $target_file)) {
			rename("$target_file","$personalizado");

				//validar la existencia 
				$preguntar = mysqli_prepare($connect,"SELECT id_imagen FROM images_informe_freezer WHERE id_informe = ? AND tipo_imagen = ? AND ubicacion = ?");
				...
				if(mysqli_stmt_num_rows($preguntar) == 0){
						$creando = mysqli_prepare($connect,"INSERT INTO images_informe_freezer (id_informe, tipo_imagen, ubicacion) VALUES (?,?,?)");
					...
					$update = mysqli_prepare($connect,"UPDATE images_informe_freezer SET ubicacion = ? WHERE id_imagen = ?");
...
//CARGAR IMAGEN PARA LA DISTRIBUCIÓN DE LOS SENSORES
	...
	$target_dir = "Evidencia_Grafica/Ot_".$id_numot."/Id_servicio_".$id_asignado."/Id_mapeo_".$id_mapeo."/Grafico_1/";
		...
		$target_file = $target_dir . basename($_FILES["imagen_2_freezer"]["name"]);
		$uploadOk = 1;
		$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

		// Check if image file is a actual image or fake image

		$check = getimagesize($_FILES["imagen_2_freezer"]["tmp_name"]);
			if($check !== false) {
				echo " Este archivo es una imagen - " . $check["mime"] . ".";
				$uploadOk = 1;
			}else{
			...
			// Check file size
			if ($_FILES["imagen_2_freezer"]["size"] > 5000000) {
				echo "Tu archivo es demasiado grande.";
				$uploadOk = 0;
			}
			// Allow certain file formats
			...
			if($uploadOk == 0) {
				...
				if(move_uploaded_file($_FILES["imagen_2_freezer"]["tmp_name"], $target_file)) {
					rename("$target_file","$personalizado");
	...
	$target_dir = "Evidencia_Grafica/Ot_".$id_numot."/Id_servicio_".$id_asignado."/Id_mapeo_".$id_mapeo."/Grafico_2/";
		...
		$target_file = $target_dir . basename($_FILES["imagen_3_freezer"]["name"]);
		$uploadOk = 1;
		$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

		// Check if image file is a actual image or fake image

		$check = getimagesize($_FILES["imagen_3_freezer"]["tmp_name"]);
			if($check !== false) {
				echo " Este archivo es una imagen - " . $check["mime"] . ".";
				$uploadOk = 1;
			}else{
			...
			// Check file size
			if ($_FILES["imagen_3_freezer"]["size"] > 5000000) {
				echo "Tu archivo es demasiado grande.";
				$uploadOk = 0;
			}
			// Allow certain file formats
			...
			if($uploadOk == 0) {
				...
				if(move_uploaded_file($_FILES["imagen_3_freezer"]["tmp_name"], $target_file)) {
					rename("$target_file","$personalizado");
