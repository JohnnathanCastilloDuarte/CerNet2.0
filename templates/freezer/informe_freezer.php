<?php 
  ...
  $ultimo_mapeo = mysqli_prepare($connect,"SELECT informe_base FROM freezer_mapeo WHERE id_asignado = ? ORDER BY fecha_registro DESC LIMIT 1");
  ...
  if($informe_base == 1){
    ...
    $existen_mapeo = mysqli_prepare($connect,"SELECT informe_base FROM freezer_mapeo WHERE id_asignado = ? AND informe_base = ?");
    ...
    $segunda_validacion = mysqli_prepare($connect,"SELECT nombre FROM informe_freezer WHERE id_asignado = $id_asignado AND id_mapeo = $id_mapeo AND tipo = 2");
    ...
    if(mysqli_stmt_num_rows($existen_mapeo) == 1 AND mysqli_stmt_num_rows($segunda_validacion) == 0){
      ...
      $consecutivo = mysqli_prepare($connect,"SELECT n_increment FROM informe_freezer WHERE id_asignado = $id_asignado  AND tipo != 2 ORDER BY id_informe_freezer DESC LIMIT 1");
