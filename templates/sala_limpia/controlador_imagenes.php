...
if($movimiento == "Leer"){
    ...
    $consultar = mysqli_prepare($connect,"SELECT id_imagen, url, tipo, nombre FROM image_sala_limpia WHERE id_asignado  = ?");
    ...
    $consultar = mysqli_prepare($connect,"SELECT url, nombre FROM image_sala_limpia WHERE id_imagen = ?");
    ...
    if(file_exists($eliminar_que)){
        ...
        $eliminar = mysqli_prepare($connect, "DELETE FROM image_sala_limpia WHERE id_imagen = ?");
