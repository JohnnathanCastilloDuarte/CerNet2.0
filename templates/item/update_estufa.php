...
if($_GET['item']){
	...
	$estufa = mysqli_prepare($connect,"SELECT a.id_estufa,  a.fabricante, a.modelo, a.n_serie, a.c_interno, a.fecha_fabricacion, a.direccion, a.ubicacion, a.voltaje, 
		...
		a.valor_seteado_tem, a.tem_min, a.tem_max,d.nombre, a.area_interna  FROM item_estufa as a, item as b, empresa as c,tipo_item as d
	...
	mysqli_stmt_bind_result($estufa, $id_estufa, $fabricante, $modelo, $n_serie, $c_interno, $fecha_fabricacion, $direccion, $ubicacion, $voltaje, $potencia, $capacidad,
		$peso, $alto, $largo, $ancho, $nombre_item, $descripcion_item, $id_empresa, $nombre_empresa, $seteado_tem, $tem_min, $tem_max,$nombre_tipo_item,$area_interna);
...
if ($_GET['pdf'] == 1) {
	...
	if($url = 'cercal.net') {
		...
		$link2  = 'https://cercal.net/CerNet2.0/templates/item/pdf/pdf/pdf_estufa.php';
		$correo = $_GET['correo'];
		header('location: ../documentacion/enviarPDF_correo.php?correo='.$correo."&link=".$link2."&conv=".$conv);		
	}else{ 	
		$link2  = 'https://localhost/CerNet2.0/templates/item/pdf/pdf/pdf_estufa.php';
		$correo = $_GET['correo'];
		header('location: ../documentacion/enviarPDF_correo.php?correo='.$correo."&link=".$link2."&conv=".$conv);
	}
}
