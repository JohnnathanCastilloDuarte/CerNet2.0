<?php 
    ...
    if($update_item){
        ...
        $actualizar_campana = mysqli_prepare($connect, "UPDATE item_campana SET tipo = ?,  marca =?, modelo=?, serie=?, codigo=?, ubicacion_interna= ?, area_interna = ?, direccion= ?,requisito_velocidad= ?,fecha_fabricacion = ? WHERE id_campana = ?");
        mysqli_stmt_bind_param($actualizar_campana, 'ssssssssssi', $tipo_campana, $marca_campana, $modelo_campana, $serie_campana, $codigo_interno_campana, $ubicacion_campana, $area_interna ,$direccion_campana, $velocidad_aire_campana, $fecha_fabricacion, $id_campana);
