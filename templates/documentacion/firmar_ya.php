...
$url_qr = "https://".$variable_url."/CerNet2.0/templates/documentacion/firmar_documentacion.php?key=".$key1."&document=".$key2;
...
if($variable_url == "cercal.net"){
  ...
  $contenido = "https://localhost/CerNet2.0/".$url;
}
 //Texto
...
if($seleccion == 1){
  ...
  */
  ...
  $consulta1 = mysqli_prepare($connect,"SELECT a.email FROM persona as a, firmantes_documentacion as b, cargo as c, departamento AS d WHERE b.id_usuario = a.id_usuario AND b.id_documento = ? AND b.id_usuario != ? AND a.id_cargo = c.id_cargo AND c.id_departamento = d.id AND d.departamento != 'Documentación' ORDER BY b.orden DESC LIMIT 1");
  mysqli_stmt_bind_param($consulta1, 'ii', $id_documentacion, $id_persona);
