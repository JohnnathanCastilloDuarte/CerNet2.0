<?php 
	...
	$primer_filtro = mysqli_prepare($connect,"SELECT id_estufaeincubadora_sensor FROM estufaeincubadora_sensor WHERE id_sensor = $id_sensor AND id_bandeja = $id_bandeja AND id_asignado = 	$id_asignado");
	mysqli_stmt_execute($primer_filtro);
	mysqli_stmt_store_result($primer_filtro);
	mysqli_stmt_bind_result($primer_filtro, $id_estufaeincubadora_sensor);
	mysqli_stmt_fetch($primer_filtro);


  $segundo_filtro = mysqli_prepare($connect,"SELECT id_estufaeincubadora_sensor FROM estufaeincubadora_datos_crudos WHERE  id_estufaeincubadora_sensor = ?");
  mysqli_stmt_bind_param($segundo_filtro, 'i',$id_estufaeincubadora_sensor);
...
//////////////////////FIN DE INGRESO DEL BACKTRAKING///////////////////////////////////////////////////////////////////////////////////////////	
		*/
		...
		$quitar = mysqli_prepare($connect,"DELETE FROM estufaeincubadora_sensor WHERE id_mapeo = ? AND id_asignado = ? AND id_bandeja = ? AND id_sensor = ?");
