 <?php 
    ...
    $id_mapeo_estufaeincubadora = $_POST['id_mapeo_estufaeincubadora'];
    $id_asignado_estufaeincubadora = $_POST['id_asignado_estufaeincubadora'];
    ...
    $query_master = mysqli_prepare($connect,"SELECT id_estufaeincubadora_sensor FROM estufaeincubadora_sensor WHERE id_mapeo = ? AND id_bandeja = ?");
    mysqli_stmt_bind_param($query_master, 'ii', $id_mapeo_estufaeincubadora, $bandejas);
...
*/
    ...
    $fechas = mysqli_prepare($connect,"SELECT fecha_inicio, hora_inicio, fecha_final, hora_final, intervalo FROM estufaeincubadora_mapeo WHERE id_mapeo = ?");
		mysqli_stmt_bind_param($fechas, 'i', $id_mapeo_estufaeincubadora);
    ...
    //buscar ot
			...
			mysqli_stmt_bind_param($consultar_servicio_1, 'i', $id_asignado_estufaeincubadora);
			...
			$directorio_carga = "datos_crudos_estufaeincubadora/id_ot_".$id_numot."/id_serv_".$id_asignado_estufaeincubadora."/id_mapeo_".$id_mapeo_estufaeincubadora."/";
...
$archivo=$directorio_carga.basename($_FILES["file_estufaeincubadora"]["name"]);
$cargaok=1;
...
$nombre_archivo=basename($_FILES["file_estufaeincubadora"]["name"]);

if(isset($_POST["file_estufaeincubadora"]))
{
  if(in_array($_FILES["file_estufaeincubadora"]["type"],$formato_valido))
  {
    $cargaok=1;
  }
  else
  {
    echo "El formato de archivo no es valido ".$_FILES["file_estufaeincubadora"]["name"];
    $cargaok=0;
  }
}


if($_FILES["file_estufaeincubadora"]["size"]>50000000)
{
...
{
  if(move_uploaded_file($_FILES['file_estufaeincubadora']['tmp_name'],$archivo))
  {
    ...
    $crudo = mysqli_prepare($connect,"UPDATE  estufaeincubadora_sensor  SET datos_crudos = ? WHERE id_mapeo = ?");
    mysqli_stmt_bind_param($crudo, 'si', $personalizado, $id_mapeo_estufaeincubadora);
    mysqli_stmt_execute($crudo);
    //echo mysqli_stmt_error($crudo);
    if($crudo){
    //echo "Listo";
    ...
    mysqli_stmt_fetch($query_master);
      ...
      if($fecha_hora_sql>=$start && $fecha_hora_sql<=$end){
          ...
          if($algoritmo_1 == 0){
              ...
              $inserta_columna_1 = mysqli_prepare($connect,"INSERT INTO estufaeincubadora_datos_crudos (id_estufaeincubadora_sensor, columna, time, temperatura, id_usuario) VALUES ( ?, ?, ?, ?, ?)");
...
mysqli_stmt_close($connect);    
  ...
  /*      
     if(isset($columna_dc1)){
        while(($column=fgetcsv($abre_archivo,10000,";","\t"))!==false){

           $fecha_hora_sql = date("Y-m-d H:i:s",strtotime($column[1].' '.$column[2]));
            //VALIDACION PARA SUMARLE  LOS SEGUNDOS DE INTERVALO A LA FECHA-HORA INICIAL Y ASI CARGAR ESTA SUMATORIA EN LUGAR DE LA FECHA-HORA DEL ARCHIVO
            if($z_1==1)
            {
              $fecha_suma=$start;
          ...
          if($fecha_hora_sql>=$start && $fecha_hora_sql<=$end){
           ...
           if(isset($columna_dc1)){
              //PROCESO PARA RETIRAR COMAS Y DEJAR VALORES CON PUNTO DECIMAL
              if(strpos($column[$columna_dc1],","))
              {
                $temperatura_1=str_replace(",",".",$column[$columna_dc1]);
              }
              else
              {
               $temperatura_1=$column[$columna_dc1];						
              }
              ...
              if($algoritmo_1 == 0){
                ...
                mysqli_stmt_bind_param($inserta_columna_1, 'iissi',$id_sensor_ultrafreezer_dc1, $col_1,$fecha_suma, $temperatura_1, $id_valida);
                mysqli_stmt_execute($inserta_columna_1);
            }

           }//cierre del if de la validación de la primera columna  
     ...
     if(isset($columna_dc2)){
       while(($column=fgetcsv($abre_archivo_2,10000,";","\t"))!==false){
        ...
        if($fecha_hora_sql>=$start && $fecha_hora_sql<=$end){
         ...
         if(isset($columna_dc2)){
        if(strpos($column[$columna_dc2],","))
        {
          $temperatura_2=str_replace(",",".",$column[$columna_dc2]);
        }
        else
        {
         $temperatura_2=$column[$columna_dc2];						
        }

        if($algoritmo_2 == 0){

          $inserta_columna_2 = mysqli_prepare($connect,"INSERT INTO ultrafreezer_datos_crudos(id_ultrafreezer_sensor, columna, time, temperatura, id_usuario) VALUES ( ?, ?, ?, ?, ?)");
          mysqli_stmt_bind_param($inserta_columna_2, 'iissi',$id_sensor_ultrafreezer_dc2, $col_2,$fecha_suma, $temperatura_2, $id_valida);
          mysqli_stmt_execute($inserta_columna_2);

        }
      }//cierre del if de la validación de la segunda columna
        $fecha_suma=date('Y-m-d H:i:s',strtotime("+$intervalo seconds",strtotime($fecha_suma)));
						$z_2=2;
        }//cierre del if
      }//cierre del while
     ...
     if(isset($columna_dc3)){
         while(($column=fgetcsv($abre_archivo_3,10000,";","\t"))!==false){
    ...
    if(isset($columna_dc3)){
      if(strpos($column[$columna_dc3],","))
      {
        $temperatura_3=str_replace(",",".",$column[$columna_dc3]);
      }
      else
      {
        $temperatura_3=$column[$columna_dc3];						
      }
      
      if($algoritmo_3 == 0){
     
        $inserta_columna_3 = mysqli_prepare($connect,"INSERT INTO ultrafreezer_datos_crudos(id_ultrafreezer_sensor, columna, time, temperatura, id_usuario) VALUES ( ?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($inserta_columna_3, 'iissi',$id_sensor_ultrafreezer_dc3, $col_3,$fecha_suma, $temperatura_3, $id_valida);
        mysqli_stmt_execute($inserta_columna_3);

      }
    }//cierre del if de la validación de la tercera columna
        $fecha_suma=date('Y-m-d H:i:s',strtotime("+$intervalo seconds",strtotime($fecha_suma)));
						$z_3=2;
   }//cierre del if
    }//cierre del while
     ...
     if(isset($columna_dc4)){
       while(($column=fgetcsv($abre_archivo_4,10000,";","\t"))!==false){
    ...
    if($fecha_hora_sql>=$start && $fecha_hora_sql<=$end){
      ...
      if(strpos($column[$columna_dc4],","))
      {
        $temperatura_4=str_replace(",",".",$column[$columna_dc4]);
      }
      else
      {
        $temperatura_4=$column[$columna_dc4];						
      }
      
      if($algoritmo_4 == 0)
      {
       
        $inserta_columna_4 = mysqli_prepare($connect,"INSERT INTO ultrafreezer_datos_crudos(id_ultrafreezer_sensor, columna, time, temperatura, id_usuario) VALUES ( ?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($inserta_columna_4, 'iissi',$id_sensor_ultrafreezer_dc4, $col_4,$fecha_suma, $temperatura_4, $id_valida);
        mysqli_stmt_execute($inserta_columna_4);
      }
