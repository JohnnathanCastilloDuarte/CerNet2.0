<?php 
	...
	$listar = mysqli_prepare($connect,"SELECT id_asignado, id_mapeo, nombre, fecha_inicio, hora_inicio, fecha_final, hora_final, 
													intervalo, temperatura_minima, temperatura_maxima FROM estufaeincubadora_mapeo WHERE id_asignado = $id_asignado");

	mysqli_stmt_execute($listar);
	mysqli_stmt_store_result($listar);
	mysqli_stmt_bind_result($listar, $id_asignado, $id_mapeo, $nombre, $fecha_inicio, $hora_inicio, $fecha_final, $hora_final, $intervalo, $temperatura_minima, 
																		$temperatura_maxima);
