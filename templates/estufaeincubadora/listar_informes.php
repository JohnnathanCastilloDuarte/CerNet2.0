<?php 
	...
	$json = array();
		...
		$informes = mysqli_prepare($connect,"SELECT a.id_informe_estufaeincubadora, a.n_increment,  a.nombre, a.tipo, a.id_mapeo, a.id_asignado, a.estado, a.fecha_registro, b.nombre, a.observacion,
			a.comentarios  FROM informe_estufaeincubadora as a, estufaeincubadora_mapeo as b WHERE  a.id_asignado = $id_asignado AND a.id_asignado = b.id_asignado AND a.id_mapeo = b.id_mapeo ORDER BY tipo DESC ");
	...
	while($row = mysqli_stmt_fetch($informes)){
			...
			$img_posicion = mysqli_prepare($connect, "SELECT ubicacion FROM images_informe_estufaeincubadora WHERE id_informe = $id_informe AND tipo_imagen = 1");
			...
			$img_graf_1 = mysqli_prepare($connect, "SELECT ubicacion FROM images_informe_estufaeincubadora WHERE id_informe = $id_informe AND tipo_imagen = 2");
			...
			$img_graf_2 = mysqli_prepare($connect, "SELECT ubicacion FROM images_informe_estufaeincubadora WHERE id_informe = $id_informe AND tipo_imagen = 3");
