<?php 
   ...
   $query_2 = mysqli_prepare($connect,"UPDATE informe_refrigerador SET observacion = ?, comentarios = ?, concepto = ? WHERE id_informe_refrigerador = ?");
    ...
    for($i = 0; $i <= count($id_mapeo_comentario);$i++){  
      $query = mysqli_prepare($connect,"UPDATE refrigerador_mapeo SET comentario = ?  WHERE id_mapeo = ?");
