	...
	$check = getimagesize($_FILES["imagen_1"]["tmp_name"]);
		...
		// if everything is ok, try to upload file
			...
			rename("$target_file","$personalizado");
				...
				$preguntar = mysqli_prepare($connect,"SELECT id_imagen FROM images_informe_refrigeradores WHERE id_informe = ? AND tipo_imagen = ? AND ubicacion = ?");
				...
				if(mysqli_stmt_num_rows($preguntar) == 0){
					...
					$update = mysqli_prepare($connect,"UPDATE images_informe_refrigeradores SET ubicacion = ? WHERE id_imagen = ?");
