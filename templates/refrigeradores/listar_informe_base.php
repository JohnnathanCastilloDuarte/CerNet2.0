...
if($seleccion == 1){
  ...
  //CONSULTAR INFORMES
  $informes = mysqli_prepare($connect,"SELECT a.id_informe_refrigerador, a.n_increment,  a.nombre, a.tipo, a.id_mapeo, a.id_asignado, a.estado, a.fecha_registro, b.nombre, a.observacion,
  a.comentarios, a.concepto  FROM informe_refrigerador as a, refrigerador_mapeo as b WHERE  a.id_asignado = $id_asignado AND a.id_asignado = b.id_asignado AND a.id_mapeo = b.id_mapeo AND a.tipo = 2 ORDER BY tipo DESC ");
  ...
  $img_posicion = mysqli_prepare($connect, "SELECT ubicacion FROM images_informe_refrigeradores WHERE id_informe = $id_informe AND tipo_imagen = 1");
  ...
  echo $convert;
    ...
    $consultar_mapeos = mysqli_prepare($connect,"SELECT id_mapeo, nombre, comentario FROM refrigerador_mapeo WHERE id_asignado = $id_asignado AND informe_base = 1");
    ...
    $consultar = mysqli_prepare($connect,"SELECT id_informe_refrigerador FROM informe_refrigerador WHERE id_asignado = $id_asignado AND tipo = 2");
    ...
    $consultar_2 = mysqli_prepare($connect,"SELECT id_imagen, ubicacion FROM images_informe_refrigeradores WHERE id_informe = $id_informe AND tipo_imagen = 4");
    ...
    $consultar_2 = mysqli_prepare($connect,"SELECT id_imagen, ubicacion FROM images_informe_refrigeradores WHERE id_informe = $id_informe AND tipo_imagen = 1");
    mysqli_stmt_execute($consultar_2);
    mysqli_stmt_store_result($consultar_2);
    mysqli_stmt_bind_result($consultar_2, $id_imagen, $url);
