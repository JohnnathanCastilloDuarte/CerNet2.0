...
do {
    /////////// VALIDACIÓN
    $consultar = mysqli_prepare($connect,"SELECT id_imagen FROM image_camara_extraccion WHERE nombre = ? AND tipo = ? AND id_asignado = ?");
    ...
    if(mysqli_stmt_num_rows($consultar) > 0){
        ...
        $creando_imagen = mysqli_prepare($connect,"INSERT INTO image_camara_extraccion (id_asignado, url, tipo, nombre) VALUES (?,?,?,?)");
