...
$(document).on("click","#modificar_mapeo_creada_automovil",function(){
  ...
  var id_mapeo = $(this).attr('data-id');
...
function validar_btn_dc(id){
  var id_mapeo = id;
  const datos = {
    id_mapeo,
    id_asignado_automovil
  }
  //alert(id_asignado_automovil+"--"+id)
  $.ajax({
    type:'POST',
    data:datos,
    url:'templates/automovil/validar_btn_dc.php',
    success:function(response){
      let button = "";
      let template = "";
      if(response == "Existe"){
        template = "<button class='btn btn-success' id='automovil_btn_dc' data-id='"+id_mapeo+"'>Archivo cargado</button>";
      }else{
        template = "<button class='btn btn-danger' id='automovil_btn_dc'  data-id='"+id_mapeo+"'>No se ha cargado el archivo</button>";
      }
...
//funcion para proceder a cargar datos crudos 

$(document).on('click','#automovil_btn_dc',function(){
  //recoje la variable del id_mapeo 
  let id_mapeo = $(this).attr('data-id');

  var URLactual = $("#es_local").val();

  if(URLactual == "Si"){
    window.open(`https://localhost/CerNet2.0/templates/datoscrudos/vistadatoscrudos.php?id_valida=${id_valida_automovil}&id_asignado=${id_asignado_automovil}&id_mapeo=${id_mapeo}`, "Datos Crudos", "width=1693px, height=1693px")


  }else{
    window.open(`https://cercal.net/CerNet2.0/templates/datoscrudos/vistadatoscrudos.php?id_valida=${id_valida_automovil}&id_asignado=${id_asignado_automovil}&id_mapeo=${id_mapeo}`, "Datos Crudos", "width=1693px, height=1693px");
  }
