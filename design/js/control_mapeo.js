...
(function(){
    ...
    let incluir_inf_base = $("#incluir_informe_base").is(":checked");
    let if_incluir = "";
				...
				<tr>
					...
					<td>
						<div style="text-align:center;">
							...
							<button class="btn btn-danger" data-id="${result.id_mapeo}" id="eliminar_mapeo_creada">X</button>
						</div>
					</td>
	...
	$(document).on('click','#eliminar_mapeo_creada',function(){
		...
		Swal.fire({
			...
			if(result.value){
				...
				$.post('templates/refrigeradores/eliminar_mapeo.php', data, function(e){
...
(function(){
	$("#btn_actualizar_mapeo").click(function(){
		...
		let nombre_mapeo = $("#nombre_mapeo").val();
		let fecha_inicio_mapeo = $("#fecha_inicio_mapeo").val();
		let hora_inicio_mapeo = $("#hora_inicio_mapeo").val();
		let minuto_inicio_mapeo = $("#minuto_inicio_mapeo").val();
		let segundo_inicio_mapeo = $("#segundo_inicio_mapeo").val();
...
function listar_sensores(a){
	...
	$.ajax({
		...
		success:function(e){
			...
			let template = "";
				...
				traer.forEach((result)=>{
					...
					<tr data-id="${result.id_sensor}">
						...
						<td>${result.tipo}</td>
						<td>
								<button class="btn btn-success" id="agregar_sensor">+</button>
								<button class="btn btn-danger" id="quitar_sensor">X</button>
						</td>				
					</tr>
					`;
			...
			$("#sensores_resultado").html(template);
		...
		success:function(e){
			...
			traer.forEach((result)=>{
				...
				template+=
					...
					<tr>
						<td>${result.nombre}</td>
						<td><button class="btn btn-success" id="buscar_sensores_asignada" data-id="${result.id_bandeja}"><i class="lnr-checkmark-circle"></i></button></td>
					</tr>
					`;
...
(function(){
	...
	$(document).on('click','#quitar_sensor',function(){
		...
		$.post('templates/refrigeradores/listar_final_mapeo.php', datos , function(e){
			...
			traer.forEach((result)=>{
				...
				if(result.datos_crudos == ""  || result.datos_crudos === null){
					button = `<button class="btn btn-danger" id="btn_cargar_datos_crudos" data-id="${result.id_refrigerador_sensor}" 
										bandeja="${result.nombre_bandeja}" sensor="${result.nombre_sensor}">Falta archivo</button>`
				}else{
					button = `<button class="btn btn-success" id="btn_cargar_datos_crudos" data-id="${result.id_refrigerador_sensor}" 
